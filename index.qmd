...
<!-- -   Dive into the data structure in `sf` -->
...
The point sfg is a paired vector with specialised class labels
...
- In practice, you don't have to decompose/ manipulate these vectors or matrices manually, existing geometrical operations (`st_xxx()`) and visualisation tools (`geom_sf()`) will do that for you.
...
missings %>% 
  ggplot() + 
  geom_errorbar(aes(xmin = .from, xmax = .to, y = fct_reorder(id, .n)), width = 0.2) + 
  theme_bw() + 
...
## Acknowledgements
  ...
  - The slides are made with [Quarto](https://quarto.org/) and are available at sherryzhang-RLadiesMelb2022.netlify.app
