...
- In practice, you don't necessary have to decompose/ manipulate these geometry types manually 
...
(cb_tm <- cb %>% 
  face_temporal() %>% 
  mutate(rolling_tmax_5 = slider::slide_dbl(tmax, mean, .after = 60, na.rm = TRUE)) )
```
...
cb_glyph <- cb %>% 
  face_temporal() %>% 
  mutate(rolling_tmax = slider::slide_dbl(tmax, mean, .after = 13, na.rm = TRUE)) %>% 
  unfold(long, lat)
...
ggplot() + 
  ...
  geom_glyph(
    ...
    aes(x_major = long, x_minor = as.numeric(date), 
        y_major = lat, y_minor = rolling_tmax),
    width = 2, height = 0.7) + 
  ggthemes::theme_map()
```
:::
